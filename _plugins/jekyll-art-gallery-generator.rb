...
module Jekyll
  ...
  class ReadYamlPage < Page
    def read_yaml(base, name, opts = {})
      begin
        merged_opts = (site ? site.file_read_opts : {}).merge(opts)
        path = File.join(base.to_s, name.to_s)
        self.content = File.read(path, **merged_opts)
        if content =~ /\A(---\s*\n.*?\n?)^((---|\.\.\.)\s*$\n?)/m
  ...
  class GalleryPage < ReadYamlPage
    ...
    def initialize(site, base, dir, gallery_name)
      ...
      sort_field = config["galleries"]["sort_field"] || "name"
        ...
        if sort_field == "timestamp"
          begin
            ...
            if date_array != nil && date_array.length > 0 and date_array[0].length > 1
              date_times[dest_image] = DateTime.strptime(date_array[0][1], "%Y:%m:%d %H:%M:%S").to_time.to_i
            end
            # puts "gtot #{date_array} date" + date_times[dest_image].to_s
          rescue Exception => e
            puts "Error getting date_time " + date_times[dest_image] + " for #{dest_image}: #{e}"
          end
        end
      end
      ...
      begin
        ...
        if gallery_config.fetch("sort_reverse", true)
          @images.reverse!
        end
      rescue Exception => e
      ...
      # normalize output name by 1) replacing non alpha digit chars to '_' 2) lower case
      header_image = gallery_config["header_image"] || best_image
      header_image.gsub!(/[^0-9A-Za-z.\-]/, "_")
      header_image.downcase!
      self.data["header_image"] = header_image

      makeThumb(site.in_dest_dir(File.join(@dir, header_image)), "header_" + header_image, config["header_thumb_size"]["x"] || 400, config["header_thumb_size"]["y"] || 400, "crop")

      self.data["header"] = {}
      self.data["header"]["image"] = File.join(@dir, "thumbs/header_" + header_image) # used in the theme
      GC.start
    end
